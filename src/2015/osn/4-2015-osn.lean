...
lemma lem1_1 : ∀ (f g : ℝ → ℝ) (c : ℝ) (X : 0 < c), fn_pos f → fn_pos g → fn_eq f g c X →
  ∀ x y : ℝ, 0 < x → f x < y → f y = f x * (g x)⁻¹ * y + (c - f x * (g x)⁻¹) * f x :=
begin
  intros f g c X h h0 h1 x y h2 h3,
  have h4 : f y = f(g x * ((y - f x) * (g x)⁻¹) + f x),
  {
    apply congr_arg,
  ...
  exact h2,
  apply real.mul_pos,
  rw ← sub_pos at h3; exact h3,
  rw inv_pos; apply h0; exact h2,
end
...
begin
  ...
  {
    cases this with m1 this; cases this with m2 this,
    use [m1, m2],
    intros a h2; split,
    exact and.left (this a h2),
    rw ← and.left (this a h2),
...
/-
  Part b: to implement the solutiion, we need to provide a proof that 0 < 2015
-/
